...
def _query_single_site(site_name, active_only=True, include_optic_locations=False):
    ...
    {{
      ...
      interface_list(filters: {{
        device: {{ site: {{ name: {{ exact: "{safe_site}" }} }} }}
    ...
    """
    body = _graphql_request(query, timeout=600)
    if "errors" in body:
...
def get_site_inventory(site_name, output_queue, active_only=True, include_optic_locations=False):
    ...
    output_queue.put(f"Querying inventory for site: {site_name}...\n\n")

    try:
        try:
            device_type_counts, device_rows, interface_type_counts, interface_rows = _query_single_site(site_name, active_only=active_only, include_optic_locations=include_optic_locations)
            ...
            summary_filename = os.path.join(DOWNLOADS_DIR, f"netbox_{site_name}_{timestamp}.csv")
            try:
                with open(summary_filename, "w", newline="") as f:
                    writer = csv.writer(f)
                    writer.writerow(["Section", "Category", "Name", "Count"])
                    writer.writerow(["Meta", "", "Site", site_name])
            ...
            detail_filename = os.path.join(DOWNLOADS_DIR, f"netbox_{site_name}_{timestamp}_devices.csv")
            try:
            ...
            if include_optic_locations and interface_rows:
                optics_filename = os.path.join(DOWNLOADS_DIR, f"netbox_{site_name}_{timestamp}_optics.csv")
                try:
...
def get_all_sites_inventory(output_queue, active_only=True, include_optic_locations=False):
    ...
    try:
        site_body = _graphql_request("{ site_list { name } }", timeout=30)
        if "errors" in site_body:
            output_queue.put(f"Could not fetch site list: {site_body['errors']}\n")
            output_queue.put(None)
            return
        all_sites = sorted(
            s["name"] for s in site_body.get("data", {}).get("site_list", []) if s.get("name")
        )
